            ...
            switch (typeName[0])
            {
                ...
                case "UC":
                    ...
                    else //in ASP.NET 1.0/1.1 load the user control based on the file
                    {
                        ...
                        if (!File.Exists(Context.Server.MapPath(ucFilename))) //original filename must have contained a "_"
                        {
                            ...
                            string[] files = Directory.GetFiles(Context.Server.MapPath(typeName[2]), filePattern);
                            if (files.Length == 1)
                                ucFilename = typeName[2] + "/" + Path.GetFileName(files[0]);
                            else
