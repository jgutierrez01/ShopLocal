...
/****************************************************************************************
	...
	Modificado:	08/16/2011 IHM
				...
				19/06/2014 AHS Agregar campos de Area de Trabajo en Soldadores
...
BEGIN
	...
	(	
	    ...
	    Campo5 NVARCHAR(100),
		EsRevision BIT,
		UltimaOrdenTrabajoEspecial NVARCHAR(50),
		NumeroControlEspecail NVARCHAR(50)
	)
	...
	(
		...
		GeneralEsRevision BIT NULL,
		GeneralOrdentrabajoEspecial NVARCHAR(50),
		GeneralNumeroControlEspecial NVARCHAR(50),
	...
	--CREATE NONCLUSTERED INDEX IX_TempSoldador_SoldadorID ON #TempSoldador(SoldadorID, Nombre, AreaTrabajo)
	...
	INSERT INTO #TempSpool
		SELECT s.SpoolID,
               ...
               Campo4,
               Campo5,
			   (CASE WHEN s.EsRevision IS NULL THEN 0 ELSE 1 END),
			   odte.NumeroOrden,
			   odts.NumeroControl
		FROM Spool s
		LEFT OUTER JOIN Cuadrante c
		on s.CuadranteID = c.CuadranteID
		LEFT OUTER JOIN OrdenTrabajoEspecialSpool odts
		ON s.UltimaOrdenTrabajoEspecial = odts.NumeroControl
		LEfT OUTER JOIN OrdenTrabajoEspecial odte
		ON odts.OrdenTrabajoEspecialID = odte.OrdenTrabajoEspecialID
		WHERE s.SpoolID IN
		(
...
INSERT INTO #TempJuntaSpool
	...
	INSERT INTO #TempJuntaSoldadura
		...
		--LEFT JOIN Soldador s on s.AreaTrabajo = s.AreaTrabajo
	...
	INSERT INTO #TempInspeccionDimensional
		SELECT DISTINCT rdd.ReporteDimensionalDetalleID,
			   ...
			   ins.NumeroEmpleado--(ins.Nombre + ' ' + ins.ApPaterno + ' ' + ins.ApMaterno)--ins.Nombre
	...
	INSERT INTO #TempAvance
		...
		(
			select	js.SpoolID,
					...
					SUM(
						CASE	WHEN (js.TipoJuntaID = @TW and jw.ArmadoAprobado = 1) or (jw.JuntaSoldaduraID is not null and jw.SoldaduraAprobada = 1) THEN 1
								ELSE 0 END
					) [JuntasSoldadas],
					s.Pdis
			from	JuntaSpool js
			...
			where	js.FabAreaID != @FIELD and -- Excluimos juntas field para armado y soldadura
			        ...
			        and (jw.JuntaWorkstatusID is null or jw.VersionJunta = 1)
					and exists
					(
						select 1
	...
	INSERT INTO #TempGeneral
		SELECT jws.JuntaWorkStatusID,
			   ...
			   ISNULL((select '1'
				...
				s.EsRevision,
				s.UltimaOrdenTrabajoEspecial,
				s.NumeroControlEspecail,
	...
	select g.GeneralJuntaWorkstatusID,
		   ...
		   g.GeneralPWHT,
		   g.GeneralEsRevision,
		   g.GeneralOrdentrabajoEspecial,
		   g.GeneralNumeroControlEspecial,
