/****** Object:  StoredProcedure [dbo].[ObtenerSeguimientoDeSpools]    Script Date: 3/19/2014 1:13:17 PM ******/
SET ANSI_NULLS ON
GO
...
/****************************************************************************************
	...
	Modificado:	JHT 23/01/2014 -- Agregar campo DiametroMayor
				...
				GTH 18/03/2014 -- Se agrego campo FechaEmision de OT
				JHT 19/03/2014 -- Agrego campo EsRevision (Spool) a tabla General 
*****************************************************************************************/
	...
	BEGIN
		...
		INSERT INTO #TempSpool
			SELECT s.SpoolID,
				   ...
				   Campo4,
				   Campo5,
				   (CASE WHEN s.EsRevision <> '' THEN 1 ELSE 0 END),
				   odte.NumeroOrden,
				   odtes.NumeroControl
			FROM Spool s
			LEFT OUTER JOIN Cuadrante c
			...
			LEFT OUTER JOIN OrdenTrabajoEspecial odte
			ON odtes.OrdenTrabajoEspecialID = odte.OrdenTrabajoEspecialID
			WHERE s.ProyectoID = @ProyectoID and
					s.SpoolID = @SpoolID
