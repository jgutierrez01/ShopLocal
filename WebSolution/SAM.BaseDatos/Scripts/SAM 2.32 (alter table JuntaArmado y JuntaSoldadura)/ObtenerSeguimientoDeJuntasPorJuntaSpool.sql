...
INSERT INTO #TempJuntaSpool
	...
	INSERT INTO #TempTratamientoPwht
		...
		WHERE	r.TipoPruebaID = 3
				and (jrtt.JuntaCampo = 0 or tjws.JuntaCampo = 0)						
		union
		SELECT DISTINCT
			   jrtt.JuntaReporteTtID,
			   jcr.JuntaCampoID,
	...
	INSERT INTO #TempAvance
		select	avance.SpoolID,
				...
				case when avance.Pdis = 0 and avance.TieneLiberacionDimensional = 1 then cast(100 as decimal(10,2)) else cast((cast(avance.JuntasArmadas as decimal(5,2)) / cast(case when avance.TotalJuntasShopSpool = 0 then 1 else avance.TotalJuntasShopSpool end as decimal(5,2))) * 100.0 as decimal(10,2)) end [PorcentajeArmado],
				case when avance.Pdis = 0 and avance.TieneLiberacionDimensional = 1 then cast(100 as decimal(10,2)) else cast((cast(avance.JuntasSoldadas as decimal(5,2)) / cast(case when avance.TotalJuntasSoldables = 0 then 1 else avance.TotalJuntasSoldables end as decimal(5,2))) * 100.0 as decimal(10,2)) end [PorcentajeSoldado]
		from
		(
			select	js.SpoolID,
					...
					wss.TieneLiberacionDimensional,
					s.Pdis
			from	JuntaSpool js
			...
			group by js.SpoolID, wss.TieneLiberacionDimensional,s.Pdis
		) avance				
