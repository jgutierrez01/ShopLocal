...
BEGIN
	...
	---------------------Agrupadores PND ------------------------------------
	CREATE TABLE #TempAgrupadoresSpoolPND (
		SpoolID INT,
		AgrupadoresSpoolPNDTiposJuntas VARCHAR(MAX) NULL,
		AgrupadoresSpoolPNDClasificacion VARCHAR(MAX) NULL,
		AgrupadoresSpoolPNDAvance VARCHAR(MAX) NULL,
		AgrupadoresSpoolPNDReparaciones INT NULL,
		AgrupadoresSpoolPNDPendientes INT NULL,
		AgrupadoresSpoolPNDReportes INT NULL
	)
	---------------------Agrupadores PND ------------------------------------
	...
	--------------------------------------------------Agrupadores PND ------------------------------------
	INSERT INTO #TempAgrupadoresSpoolPND
	SELECT
	sp.SpoolID,
	(
		SELECT
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'BW' from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 4 FOR XML PATH(''), TYPE)),
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'INS'from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 3 FOR XML PATH(''), TYPE)),
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'SW' from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 5 FOR XML PATH(''), TYPE)),
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'TW' from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 2 FOR XML PATH(''), TYPE)),
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'SOF' from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 16 FOR XML PATH(''), TYPE)),
			(SELECT (SELECT COUNT(TipoJuntaID) AS 'TH' from JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND TipoJuntaID = 1 FOR XML PATH(''), TYPE))
		--FROM Spool s
		--WHERE s.SpoolID = sp.SpoolID
		FOR XML RAW('Tipos'), ELEMENTS, ROOT('Juntas')
	),
	(
		SELECT
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 4) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'BW' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 4 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'BW' FOR XML PATH(''), TYPE)
				END
			),
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 3) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'INS' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 3 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'INS' FOR XML PATH(''), TYPE)
				END
			),
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 5) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'SW' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 5 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'SW' FOR XML PATH(''), TYPE)
				END
			),
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 2) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'TW' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 2 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'TW' FOR XML PATH(''), TYPE)
				END
			),
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 16) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'SOF' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 16 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'SOF' FOR XML PATH(''), TYPE)
				END
			),
			(SELECT CASE WHEN 
					(SELECT Clasificacion FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 1) IS NOT NULL 
				THEN
					(SELECT Clasificacion AS 'TH' FROM AgrupadoresSpoolPND ags WHERE ags.SpoolID = sp.SpoolID AND ags.TipoJuntaID = 1 FOR XML PATH('') ,TYPE)
				ELSE
					(SELECT '0' AS 'TH' FOR XML PATH(''), TYPE)
				END
			)
		--FROM Spool s
		--WHERE s.SpoolID = sp.SpoolID
	...
	(
		SELECT 
		(SELECT CASE WHEN
			(SELECT COUNT(jrp.JuntaReportePndID) FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 ) > 0
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'BW' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
			)
		ELSE
			(SELECT '0' AS 'BW' FOR XML PATH(''), TYPE)
		...
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'INS' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
		...
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'SW' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
		...
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'TW' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
		...
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'SOF' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
		...
		THEN
			(
				SELECT COUNT(jrp.JuntaReportePndID) AS 'TH' FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
				INNER JOIN JuntaWorkstatus jws ON jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
				INNER JOIN JuntaSpool js ON jws.JuntaSpoolID = js.JuntaSpoolID
				WHERE js.SpoolID = sp.SpoolID AND rp.TipoPruebaID = 1 AND js.TipoJuntaID = 1 AND jrp.Aprobado = 1 AND jws.JuntaFinal = 1 
				FOR XML PATH('') ,TYPE
	...
	(
		SELECT COUNT(JuntaSpoolID) FROM JuntaSpool js WHERE js.SpoolID = sp.SpoolID AND js.Etiqueta LIKE 'R%'
	),
	(
		SELECT (COUNT(js.JuntaSpoolID) - COUNT(jsol.JuntaSoldaduraID)) AS Falantes
		FROM JuntaSpool js INNER JOIN JuntaWorkstatus jws ON js.JuntaSpoolID = jws.JuntaSpoolID
		LEFT JOIN JuntaSoldadura jsol ON jws.JuntaWorkstatusID = jsol.JuntaWorkstatusID
		WHERE js.SpoolID = sp.SpoolID  AND jws.JuntaFinal = 1 AND js.Etiqueta LIKE 'R%'
	),
	(
		SELECT COUNT(js.JuntaSpoolID) FROM JuntaSpool js INNER JOIN JuntaWorkstatus jws ON js.JuntaSpoolID = jws.JuntaSpoolID
		WHERE js.SpoolID = sp.SpoolID
		AND js.Etiqueta LIKE 'R%'
		AND jws.JuntaWorkstatusID IN (SELECT JuntaWorkstatusID FROM JuntaReportePnd)
		AND jws.JuntaWorkstatusID NOT IN (SELECT JuntaWorkstatusID FROM JuntaSoldadura)
	)
	FROM Spool sp
	WHERE sp.SpoolID IN  (SELECT SpoolID FROM #TempSpool)
...
--INSERT PINTURA
	...
	--DESPLEGAR TABLA
		...
		select g.GeneralSpoolID,
		   ...
		   tmph.PruebaHidroPresion,
			...
			convert(varchar, g.GeneralFechaLocalizacion, 103) AS GeneralFechaLocalizacion,
			tasp.AgrupadoresSpoolPNDTiposJuntas,
			tasp.AgrupadoresSpoolPNDClasificacion,
			tasp.AgrupadoresSpoolPNDAvance,
			tasp.AgrupadoresSpoolPNDReparaciones,
			tasp.AgrupadoresSpoolPNDPendientes,
			tasp.AgrupadoresSpoolPNDReportes
	from #TempGeneral g
	left join
	(
	...
	LEFT JOIN #TempAgrupadoresSpoolPND tasp ON g.GeneralSpoolID = tasp.SpoolID
