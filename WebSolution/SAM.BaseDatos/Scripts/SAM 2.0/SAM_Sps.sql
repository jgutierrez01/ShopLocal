	...
	select  nu.ItemCodeID,
		   ...
		   (
				...
				where	num.NumeroUnicoID = nu.NumeroUnicoID
						and num.TipoMovimientoID in (7,11)
						and num.Estatus = 'A'
			...
			(
				...
				from NumeroUnicoMovimiento num
				where	num.NumeroUnicoID = nu.NumeroUnicoID
						and num.TipoMovimientoID not in (2,18,5,9)
						and num.Estatus = 'A'
						and exists
						(
							select 1
...
group by d.ItemCodeID, d.Diametro1, d.Diametro2
	...
	SELECT distinct ic.ItemCodeID,
		   ...
		   isnull(nu.InventarioBuenEstado,0) [InventarioBuenEstado],
		   isnull(nu.InventarioCongelado,0) [InventarioCongelado],
		   ...
		   isnull(nu.InventarioFisico,0) [InventarioFisico],
		   isnull(nu.TotalEntradaOtrosProcesos,0) [TotalEntradaOtrosProcesos],
		   isnull(nu.TotalDespachado,0) [TotalDespachado],
		   case when (isnull(nu.TotalCorte,0) - isnull(nu.TotalDespachado,0))<0 then 0
		   else isnull(nu.TotalCorte,0) - isnull(nu.TotalDespachado,0) end [TotalCorteSinDespacho],
		   ...
		   isnull(iceq.InventarioBuenEstadoEquivalente,0) [InventarioBuenEstadoEquivalente],
		   isnull(de.CantidadDespachadaEquivalente,0) [CantidadDespachadaEquivalente],
		   isnull(de.CantidadCongeladaEquivalente,0) [CantidadCongeladaEquivalente]
