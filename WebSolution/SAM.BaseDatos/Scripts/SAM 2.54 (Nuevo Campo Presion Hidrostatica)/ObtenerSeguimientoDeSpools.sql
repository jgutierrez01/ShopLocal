	...
	(
		...
		(
			SELECT	tjs.SpoolID,
					...
					SUM(
                                                CASE        WHEN tjss.TipoJuntaID = @TH or (tjwss.JuntaWorkstatusID is not null and tjwss.VersionJunta != 1) THEN 0
                                                                ELSE 1 END
                                        ) [TotalJuntasParaSoldar],
					SUM(
                                                CASE        WHEN tjwss.JuntaSoldaduraID is not null and tjwss.SoldaduraAprobada = 1 and tjwss.VersionJunta = 1 THEN 1
                                                                ELSE 0 END
                                        )  [Soldadas],
			...
			WHERE	tjs.FabAreaID = @ShopFabAreaID
					and (tjws.JuntaWorkstatusID is null or tjws.VersionJunta = 1)
					and (tjwss.JuntaWorkstatusID is null or tjwss.VersionJunta = 1)
					AND tjs.SpoolID IN
					(
