...
BEGIN
	...
	LEFT JOIN #TempMaterialSpool ms1 on ms1.SpoolID = CASE WHEN ISNUMERIC(js.EtiquetaMaterial1) = 1 THEN js.SpoolID ELSE jca.Spool1ID END and (ms1.Etiqueta = js.EtiquetaMaterial1 or ms1.ValorNumericoEtiqueta = (CASE WHEN ISNUMERIC(js.EtiquetaMaterial1) = 1 THEN CAST(js.EtiquetaMaterial1 AS decimal(9,3)) ELSE jca.EtiquetaMaterial1 END))
	LEFT JOIN #TempMaterialSpool ms2 on ms2.SpoolID = CASE WHEN ISNUMERIC(js.EtiquetaMaterial2) = 1 THEN js.SpoolID ELSE jca.Spool2ID END and (ms2.Etiqueta = js.EtiquetaMaterial2 or ms2.ValorNumericoEtiqueta = (CASE WHEN ISNUMERIC(js.EtiquetaMaterial2) = 1 THEN CAST(js.EtiquetaMaterial2 AS decimal(9,3)) ELSE jca.EtiquetaMaterial2 END))
