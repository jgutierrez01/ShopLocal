...
BEGIN
	...
	(	
		...
		(
				...
				GeneralReferenciaSeguimiento VARCHAR(500) NULL
		)
	...
	--------------------------------     AGRUPADORES      -------------------------------------------
		...
		INSERT INTO #TempJuntaReportePnd
				SELECT DISTINCT jrpnd.JuntaReportePndID,
						   ...
						   (SELECT TOP 1 JuntaSeguimientoID1 FROM JuntaReportePnd WHERE JuntaWorkstatusID = jw.JuntaWorkStatusAnterior
								AND  Aprobado = 0 ORDER BY JuntaReportePndID ASC),
						   (SELECT TOP 1 JuntaSeguimientoID2 FROM JuntaReportePnd WHERE JuntaWorkstatusID = jw.JuntaWorkStatusAnterior
								AND  Aprobado = 0 ORDER BY JuntaReportePndID ASC)
				...
				SELECT jws.JuntaWorkStatusID,
						   ...
						   ISNULL((select '1'
							...
							s.SistemaPintura,
								...
								s.ClasifInox,
								s.FechaLocalizacion,
								(SELECT odts.NumeroControl + '-' + jws.EtiquetaJunta + ', ' FROM JuntaReportePnd jrp 
									INNER JOIN JuntaWorkstatus jws on jrp.JuntaWorkstatusID = jws.JuntaWorkstatusID
									INNER JOIN OrdenTrabajoSpool odts on jws.OrdenTrabajoSpoolID = odts.OrdenTrabajoSpoolID
								WHERE jrp.JuntaSeguimientoID1 = js.JuntaSpoolID OR jrp.JuntaSeguimientoID2 = js.JuntaSpoolID
								FOR XML PATH(''))
...
INSERT INTO #TempGeneral (
			...
			SELECT
				...
				case when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneOT = 0) then 0 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 1) then 100 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 0) then 0 else isnull(grupo.PorcSoldado,0) end
			FROM Spool s
				LEFT JOIN Proyecto p on s.ProyectoID = p.ProyectoID
				LEFT JOIN SpoolHold sh on sh.SpoolID = s.SpoolID
...
-------------------------------------------------------------------------------------------------------------------------------
		...
		select g.GeneralJuntaWorkstatusID,
				   ...
				   g.AgrupadoresSpoolUltimoProceso,
					...
					g.GeneralReferenciaSeguimiento,
