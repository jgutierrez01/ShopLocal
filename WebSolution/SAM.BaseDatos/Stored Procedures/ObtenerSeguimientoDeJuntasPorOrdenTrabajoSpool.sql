...
Modificado:     28/01/2011 IHM, 14/03/2011 MMG,
				...
				27/03/2013 BGG
				07/02/2014 JHT
				17/02/2014 JHt Prueba Hidrostatica
				11/03/2014 JHT Liberación Calidad
				18/03/2014 GTG Agrego Fecha Emision de OT
				10/04/2014 Vigencia Aduana 
                                Bernardo Garza, 13/05/2014 -- cambios en cálculo de % armado y soldadura
				05/06/2014 GTG se agrego el campo RequierePwht en JuntaSpool
				06/06/2014 JHT Nuevo Campo Fecha Liberacion Material /// Nuevo Campo Liberado Pintura
				13/06/2014 JHT Nuevo campo Fecha Ok PND
				01/07/2014 GTG -- Modificacion de insert into #TempInspeccionDimensional
				07/08/2014 GTG -- Nuevo Modulo PruebaNeumatica
				01/07/2014 GTG nuevos campos RequierePruebaHidrostatica y RequierePruebaNeumatica
				14/05/2014 JHT -- Nuevo Campo Presion para pruebas hidrostaticas

				GTG 23/07/2014 -- Nuevos campos en modulo general
				04/08/2014 JHT -- Nuevos campos de agrupadores PND, Soportes, Reparaciones
				05/08/2014 JHT -- Cambios a los campos y tablas de agrupadores por junta
				25/07/2014 JHT -- cambio a la columa de requiere prueba hidrostatica
				...
				20/08/2014 JHT -- Nuevos campos juntas de seguimientos en reporte RT (JuntaReportePND)
				26/08/2014 JHT -- cambio Fecha Preparacion a Folio Preparacion Embarques
				19/06/2014 AHS Nuevo campo Area de Trabajo en Soldadura
				19/06/2014 AHS Agregar campo de Area de Trabajo en Tuberos
				03/07/2014 AHS Agregar campo de Inspector en Inspeccion Dimensional
*****************************************************************************************/
...
BEGIN
	...
	-------------------- AGRUPADORES --------------------------------------
		...
		(        
				...
				JuntaFinal BIT,
				JuntaWorkStatusAnterior INT NULL
		)
		...
		(        
				...
				AreaTrabajoRelleno NVARCHAR(100),
				AreaTrabajoRaiz NVARCHAR(100)
		)
		...
		(        
				...
				Observaciones NVARCHAR(500),
				FechaModificacion DATETIME,
				JuntaSeguimientoID1 INT NULL,
				JuntaSeguimientoID2 INT NULL
		)
		...
		(        
				...
				FechaModificacion DATETIME,
				InspeccionDimensionalInspector NVARCHAR(100)
		)
		...
		(        
				...
				InspeccionVisualInspector NVARCHAR(100)	               
		)
		...
		(        
				...
				PruebaRTJuntaSeguimiento1 VARCHAR(500) NULL,
				PruebaRTJuntaSeguimiento2 VARCHAR(500) NULL,
				PruebaRTReferenciaSeguimiento VARCHAR(500)
		)
	...
	(	
		...
		(
				...
				EmbarqueFolioPreparacion VARCHAR(50),
	...
	(	
		...
		(
				...
				GeneralCortador VARCHAR(500),
				GeneralDespachador VARCHAR(500),
	...
	WHERE NOT EXISTS
		...
		INSERT INTO #TempOrdenTrabajoMaterial
				SELECT otm.OrdenTrabajoMaterialID,
						   ...
						   otm.TieneDespacho,
						   otm.EsAsignado,
						   (Select cr.NumeroEmpleado 
								from Cortador cr
								INNER JOIN Corte c ON cr.CortadorID = c.CortadorID
								INNER JOIN CorteDetalle cd ON c.CorteID = cd.CorteID
								WHERE cd.CorteDetalleID = otm.CorteDetalleID ),
							(SELECT dr.NumeroEmpleado
								FROM Despachador dr
								INNER JOIN Despacho d ON dr.DespachadorID = d.DespachadorID
								WHERE d.DespachoID = otm.DespachoID)                        
	...
	INSERT INTO #TempAgrupadoresPorJunta		
		...
		WHERE JuntaSpoolID IN (SELECT JuntaSpoolID FROM #TempJuntaSpool)
	--------------------------------     AGRUPADORES      -------------------------------------------
	...
	INSERT INTO #TempJuntaSoldadura
		SELECT DISTINCT js.JuntaSoldaduraID,
						...
						fm2.Nombre,
						js.Observaciones,
						ssr.AreaTrabajoRelleno,
						ssrr.AreaTrabajoRaiz
		...
		LEFT JOIN(
			SELECT jsd.JuntaSoldaduraID, 
				REPLACE(RTRIM((SELECT DISTINCT s.AreaTrabajo + '' + CAST('' AS NVARCHAR(MAX)) + ' ' 
			FROM JuntaSoldaduraDetalle jsd1
			INNER JOIN Soldador s on s.SoldadorID = jsd1.SoldadorID
			WHERE (jsd1.JuntaSoldaduraID = jsd.JuntaSoldaduraID) and jsd1.TecnicaSoldadorID  = 2
			FOR XML PATH (''))),' ',', ') AS AreaTrabajoRelleno
			FROM JuntaSoldaduraDetalle jsd
			GROUP BY jsd.JuntaSoldaduraID
		) ssr on ssr.JuntaSoldaduraID = js.JuntaSoldaduraID
		LEFT JOIN(
			SELECT jsd.JuntaSoldaduraID, 
				   REPLACE(RTRIM((SELECT DISTINCT s.Codigo + '' + CAST('' AS NVARCHAR(MAX)) + ' ' 
			FROM JuntaSoldaduraDetalle jsd1
		...
		LEFT JOIN(
			...
			INNER JOIN Soldador s on s.SoldadorID = jsd1.SoldadorID
			WHERE (jsd1.JuntaSoldaduraID = jsd.JuntaSoldaduraID) and jsd1.TecnicaSoldadorID  = 1
			FOR XML PATH (''))),' ',', ') AS AreaTrabajoRaiz
			FROM JuntaSoldaduraDetalle jsd
			GROUP BY jsd.JuntaSoldaduraID
		) ssrr on ssrr.JuntaSoldaduraID = js.JuntaSoldaduraID		
		LEFT JOIN(
			SELECT jsd.JuntaSoldaduraID, 
		...
		LEFT JOIN(
			...
			FROM JuntaCampoSoldaduraDetalle jcsd
			GROUP BY jcsd.JuntaCampoSoldaduraID
		) ssr on ssr.JuntaCampoSoldaduraID = jcs.JuntaCampoSoldaduraID
		LEFT JOIN(
			SELECT jcsd.JuntaCampoSoldaduraID, 
				   REPLACE(RTRIM((SELECT DISTINCT s.Codigo + '' + CAST('' AS NVARCHAR(MAX)) + ' ' 
			FROM JuntaCampoSoldaduraDetalle jcsd1
		...
		LEFT JOIN(
			SELECT jcsd.JuntaCampoSoldaduraID, 
				REPLACE(RTRIM((SELECT DISTINCT s.AreaTrabajo + '' + CAST('' AS NVARCHAR(MAX)) + ' ' 
			FROM JuntaSoldaduraDetalle jcsd1
			INNER JOIN Soldador s on s.SoldadorID = jcsd1.SoldadorID
			WHERE (jcsd1.JuntaSoldaduraID = jcsd.JuntaCampoSoldaduraID) and jcsd1.TecnicaSoldadorID  = 1
			FOR XML PATH (''))),' ',', ') AS AreaTrabajoRaiz
			FROM JuntaCampoSoldaduraDetalle jcsd
			GROUP BY jcsd.JuntaCampoSoldaduraID
		) ssrr on ssrr.JuntaCampoSoldaduraID = jcs.JuntaCampoSoldaduraID	
		LEFT JOIN(
			SELECT jcsd.JuntaCampoSoldaduraID, 
				   REPLACE(RTRIM((SELECT co.Codigo + '' + CAST('' AS NVARCHAR(MAX)) + ' ' 
			FROM JuntaCampoSoldaduraDetalle jcsd1
	...
	LEFT JOIN JuntaInspeccionVisual jiv on jws.JuntaWorkStatusID = jiv.JuntaWorkstatusID ------------------------------------
	LEFT JOIN Inspector ins on ins.PatioID = jiv.InspectorID -----------------------------------
	...
	INSERT INTO #TempInspeccionDimensional
		...
		LEFT JOIN Inspector ins on ins.InspectorID=rdd.InspectorID	
		right JOIN(
	...
	INSERT INTO #TempJuntaReportePnd
		SELECT DISTINCT jrpnd.JuntaReportePndID,
			   ...
			   jrpnd.JuntaSeguimientoID1,
			   jrpnd.JuntaSeguimientoID2
	...
	INSERT INTO #TempPruebaRT
		SELECT DISTINCT jrpnd.JuntaReportePndID,
			   ...
			   (SELECT NumeroControl + '-' + js.Etiqueta FROM OrdenTrabajoSpool odts INNER JOIN JuntaSpool js ON odts.SpoolID = js.SpoolID
					WHERE js.JuntaSpoolID = jrpnd.JuntaSeguimientoID1),
			   (SELECT NumeroControl + '-' + js.Etiqueta FROM OrdenTrabajoSpool odts INNER JOIN JuntaSpool js ON odts.SpoolID = js.SpoolID
					WHERE js.JuntaSpoolID = jrpnd.JuntaSeguimientoID2),
				(SELECT TOP 1 odtsp.NumeroControl + '-' + jsp.Etiqueta FROM JuntaSpool jsp 
				INNER JOIN OrdenTrabajoSpool odtsp ON jsp.SpoolID = odtsp.SpoolID
				INNER JOIN JuntaWorkstatus jwss ON jsp.JuntaSpoolID = jwss.JuntaSpoolID
				WHERE jsp.JuntaSpoolID = jrpnd.JuntaSeguimientoID1 or jsp.JuntaSpoolID = jrpnd.JuntaSeguimientoID2)
	...
	INSERT INTO #TempEmbarque
			SELECT DISTINCT es.EmbarqueSpoolID,
				   ...
				   ws.EmbarqueEtiqueta,
				   ws.FechaEtiqueta,
				   (CASE WHEN ws.FechaPreparacion IS NOT NULL THEN CONVERT(CHAR(10), ws.FechaPreparacion,126) ELSE '' END) 
						+ (CASE WHEN ws.FolioPreparacion > 0 THEN '-' + replicate ('0',(3 - len(ws.FolioPreparacion))) + convert(varchar, ws.FolioPreparacion)
															  ELSE '' END),
				   e.FechaEmbarque,
				   e.FechaEstimada,
				   e.NumeroEmbarque,
...
INSERT INTO #TempGeneral (
				...
				PorcentajeArmado,
				PorcentajeSoldado,
				GeneralDiametroPlano,-----------
				GeneralUltimaLocalizacion,
				ObservacionesHold,
				FechaHold,
				GeneralFamiliaAcero1,
				GeneralFamiliaAcero2,
				GeneralFamiliaMaterial1,
				GeneralFamiliaMaterial2,
				GeneralEspecificacion,
				Campo1,
				Campo2,
				Campo3,
				Campo4,
			...
			SELECT
				(SELECT p.Nombre FROM Proyecto p WHERE p.ProyectoID = s.ProyectoID),
				ot.NumeroOrden,
				ot.FechaOrden,
				s.Nombre,
				s.Prioridad,
				...
				'0',
				'0',
				case when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneOT = 0) 
					then 0 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 1) 
								then 100 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 0) 
												then 0 
												else isnull(grupo.PorcArmado,0) 
				end,
				case when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneOT = 0) 
					then 0 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 1) 
								then 100 when EXISTS(select * from #SpoolTramoRecto tr where tr.SpoolID = s.SpoolID and tr.TieneDespacho = 0) 
												then 0 else isnull(grupo.PorcSoldado,0) 
				end,
				s.DiametroPlano,
				(SELECT c.Nombre FROM Cuadrante c WHERE c.CuadranteID = s.CuadranteID),
				SpoolHoldHistorial.Observaciones,
				SpoolHoldHistorial.FechaHold,
				fa1.Nombre,
				CASE WHEN fa2.Nombre IS NOT NULL 
					 THEN fa2.Nombre 
					 ELSE fa1.Nombre 
				END,
				fm1.Nombre,
				CASE WHEN fm2.Nombre IS NOT NULL THEN fm2.Nombre ELSE fm1.Nombre END,
				s.Especificacion,
				s.Campo1,
				s.Campo2,
				s.Campo3,
				s.Campo4,
				s.Campo5,
				(SELECT ws.FechaLiberacionCalidad FROM WorkstatusSpool ws WHERE ws.OrdenTrabajoSpoolID = ots.OrdenTrabajoSpoolID),
				(SELECT u.Nombre + ' '  + u.ApPaterno + ' '  + u.ApMaterno 
						FROM Usuario u INNER JOIN WorkstatusSpool ws ON u.UserId = ws.UsuarioLiberacionCalidad  
						WHERE UserId = ws.UsuarioliberacionCalidad AND ws.OrdenTrabajoSpoolID = ots.OrdenTrabajoSpoolID),
				(SELECT ws.FechaLiberacionMateriales FROM WorkstatusSpool ws WHERE ws.OrdenTrabajoSpoolID = ots.OrdenTrabajoSpoolID),
				(SELECT u.Nombre + ' '  + u.ApPaterno + ' '  + u.ApMaterno
						FROM Usuario u INNER JOIN WorkstatusSpool ws ON u.UserId = ws.UsuarioLiberacionMateriales 
						WHERE UserId = ws.UsuarioLiberacionMateriales AND ws.OrdenTrabajoSpoolID = ots.OrdenTrabajoSpoolID)
			FROM Spool s
				LEFT JOIN SpoolHold sh on sh.SpoolID = s.SpoolID
				...
				LEFT JOIN #TempAvance grupo on s.SpoolID = grupo.SpoolID
				LEFT JOIN FamiliaAcero fa1 on fa1.FamiliaAceroID = s.FamiliaAcero1ID
				LEFT JOIN FamiliaMaterial fm1 on fa1.FamiliaMaterialID = fm1.FamiliaMaterialID
...
-------------------------------------------------------------------------------------------------------------------------------
		...
		select g.GeneralJuntaWorkstatusID,
				   ...
				   ja.AreaTrabajo [AreaTrabajoTubero], 	--AQUI VA A IR EL AREA DE TRABAJO
				   ...
				   js.AreaTrabajoRaiz,		--AQUI VA A IR EL AREA DE TRABAJO
				   js.SoldaduraSoldadorRelleno,
				   js.AreaTrabajoRelleno,	--AQUI VA A IR EL AREA DE TRABAJO
				   ...
				   iv.InspeccionVisualInspector,   --Inspector
				   ...
				   id.InspeccionDimensionalInspector,   --Inspector                  
				   ...
				   prt.PruebaRTJuntaSeguimiento1,
				   prt.PruebaRTJuntaSeguimiento2,
				   prt.PruebaRTReferenciaSeguimiento,
		...
		(                        
				select        tmp.PruebaRTFechaRequisicion,
								...
								tmp.PruebaRTJuntaSeguimiento1,
								tmp.PruebaRTJuntaSeguimiento2,
								tmp.PruebaRTReferenciaSeguimiento
	...
	LEFT JOIN
		...
		(
				SELECT        tmp.InspeccionDimensionalFecha,
								...
								tmp.InspeccionDimensionalInspector
