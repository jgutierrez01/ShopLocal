...
BEGIN	
	...
	CREATE TABLE #WorkstatusSpool(
		...
		TienePruebaHidrostatica BIT,
		NumeroReportePruebaHidrostatica NVARCHAR(50)
	)
...
CREATE TABLE #MTR(
	...
	LEFT JOIN WorkstatusSpool wss
		on	ots.OrdenTrabajoSpoolID = wss.OrdenTrabajoSpoolID
	...
	SELECT	s.SpoolID, 
			...
			LiberadoPintura, 
			case when (SELECT LDCertificado FROM ProyectoDossier where ProyectoID = @proyectoID) = 1 then NumeroReporte
			else s.NumeroControl end, 
			...
			Embarcado,
			NumeroEmbarque,
			case when exists(select * from SpoolReportePnd srp 
			                 where srp.WorkstatusSpoolID = ws.WorkstatusSpoolID and srp.Aprobado = 1) then 1 else 0 end,
			(select rsp.NumeroReporte from SpoolReportePnd srp
			 inner join ReporteSpoolPnd rsp on srp.ReporteSpoolPndID = rsp.ReporteSpoolPndID
			 where srp.WorkstatusSpoolID = ws.WorkstatusSpoolID and srp.Aprobado = 1) 
	...
	FROM	#JuntaWorkstatus j 
	INNER JOIN JuntaSoldadura js on js.JuntaWorkstatusID = j.JuntaWorkStatusID
	INNER JOIN Wps w on w.WpsID = js.WpsRellenoID
	) A
	...
	INSERT INTO #MTR
		SELECT	otm.OrdenTrabajoMaterialID
				,query.SpoolID
				,case when (SELECT MTRCertificado FROM ProyectoDossier where ProyectoID = @proyectoID) = 1 then c.NumeroCertificado
					else c.NumeroColada end
		FROM OrdenTrabajoMaterial otm
		JOIN (SELECT s.SpoolID, ots.OrdenTrabajoSpoolID	FROM #Spools s
	...
	INSERT INTO #MTRSoldadura
		SELECT DISTINCT s.SpoolID
		,c.Codigo
		FROM Consumible c
		JOIN JuntaSoldaduraDetalle jsd on c.ConsumibleID = jsd.ConsumibleID
		JOIN JuntaSoldadura js on jsd.JuntaSoldaduraID = js.JuntaSoldaduraID
		JOIN JuntaWorkstatus jw on js.JuntaWorkstatusID = jw.JuntaWorkstatusID
		JOIN JuntaSpool jspool on jw.JuntaSpoolID = jspool.JuntaSpoolID
		JOIN #Spools s on s.SpoolID = jspool.SpoolID
