...
/****************************************************************************************
	...
	Modificado:	JHT 23/01/2014 -- Agregar campo DiametroMayor
				...
				JHT 03/09/2014 -- Nuevo modulo Agrupadores PND
				JHT 14/10/2014 -- Modificacion de campos de agrupadores PND + nueva Columna
*****************************************************************************************/
...
BEGIN
	...
	CREATE TABLE #TempAgrupadoresSpoolPND (
		...
		AgrupadoresSpoolPNDJuntasSeguimiento INT NULL
	)
	---------------------Agrupadores PND ------------------------------------
	...
	(
		...
		EmbarqueFolioPreparacion VARCHAR(20),
	...
	--------------------------------------------------Agrupadores PND ------------------------------------
DECLARE @sID INT, @fabAreaId INT, @TipoPruebaRTPOSTId INT

SET @fabAreaId = (SELECT FabAreaID FROM FabArea WHERE Codigo = 'SHOP')
SET @TipoPruebaRTPOSTId = (SELECT TipoPruebaID FROM TipoPrueba WHERE Nombre = 'RT (POST-TT)')
...
DECLARE TiposJUntas CURSOR FOR
	SELECT SpoolID FROM #TempSpool
OPEN TiposJUntas
FETCH NEXT FROM TiposJUntas INTO @sID
WHILE @@Fetch_Status = 0 BEGIN
	--SELECT @sID
	INSERT INTO @Tabla
		SELECT tj.TipoJuntaID,
				@sID,
				tj.Codigo,
				(
					SELECT COUNT(JuntAspoolID) FROm JuntaSpool js 
					WHERE js.SpoolID = @sID 
					AND js.TipoJuntaID = tj.TipoJuntaID
					AND js.FabAreaID = @fabAreaId
				)
		FROM TipoJunta tj
		WHERE tj.Codigo IN ('TW', 'BW', 'SW', 'TH')
		ORDER BY tj.TipoJuntaID

	INSERT INTO @Tabla
		SELECT (SELECT TipoJuntaID FROM TipoJunta WHERE Codigo = 'INS'),
			   @sID,
			   'INS',
			   (
					SELECT COUNT(JuntAspoolID) FROm JuntaSpool js 
					WHERE js.SpoolID = @sID
					AND js.TipoJuntaID IN 
					(
						SELECT TipoJuntaID FROM TipoJunta
						WHERE Codigo NOT IN ('TW', 'BW', 'SW', 'TH', 'SOP', 'FL', 'FW', 'SOB', 'SOE', 'STU', 'SOT')
					)
					AND js.FabAreaID = @fabAreaId
			   )

	INSERT INTO @Tabla
		SELECT (SELECT TipoJuntaID FROM TipoJunta WHERE Codigo = 'SOP'),
			   @sID,
			   'SOP',
			   (
					SELECT COUNT(JuntAspoolID) FROm JuntaSpool js 
					...
					(
						SELECT TipoJuntaID FROM TipoJunta
						WHERE Codigo IN ('SOP', 'FL', 'FW', 'SOB', 'SOE', 'STU', 'SOT')
					)
					AND js.FabAreaID = @fabAreaId
			   )
...
END
CLOSE TiposJUntas
DEALLOCATE TiposJUntas

DECLARE @TablaClasificacion TABLE(
	TipoId INT,
	SpoolID INT,
	Codigo CHAR(5),
...
DECLARE ClasificacionJuntas CURSOR FOR
	SELECT SpoolID FROM #TempSpool
OPEN ClasificacionJuntas
FETCH NEXT FROM ClasificacionJuntas INTO @sID
WHILE @@Fetch_Status = 0 BEGIN
	--SELECT @sID
	INSERT INTO @TablaClasificacion
		SELECT tj.TipoJuntaID,
				@sID,
				tj.Codigo,
				(
					CASE WHEN (SELECT Clasificacion FROM AgrupadoresSpoolPND WHERE SpoolID = @sID AND TipoJuntaID = tj.TipoJuntaID) IS NOT NULL THEN
						(SELECT Clasificacion FROM AgrupadoresSpoolPND WHERE SpoolID = @sID AND TipoJuntaID = tj.TipoJuntaID)
					ELSE
						'0'
					END
				)
		...
		SELECT (SELECT TipoJuntaID FROM TipoJunta WHERE Codigo = 'INS'),
				...
				(
					CASE WHEN 
						(
							SELECT COUNT(Clasificacion) FROM AgrupadoresSpoolPND 
							WHERE SpoolID = @sID AND TipoJuntaID IN (SELECT TipoJuntaID FROM TipoJunta
																		WHERE Codigo NOT IN ('TW', 'BW', 'SW', 'TH', 'SOP', 'FL', 'FW', 'SOB', 'SOE', 'STU', 'SOT'))
						) > 0 THEN
						(
							SELECT Clasificacion 
							FROM AgrupadoresSpoolPND 
							WHERE SpoolID = @sID 
							AND TipoJuntaID IN 
							(
								SELECT TipoJuntaID FROM TipoJunta
		...
		SELECT (SELECT TipoJuntaID FROM TipoJunta WHERE Codigo = 'SOP'),
				...
				(
					...
					(
						SELECT COUNT(Clasificacion) FROM AgrupadoresSpoolPND 
						WHERE SpoolID = @sID AND TipoJuntaID IN (SELECT TipoJuntaID FROM TipoJunta WHERE Codigo IN ('SOP', 'FL', 'FW', 'SOB', 'SOE', 'STU', 'SOT'))
					) > 0 THEN
						(
							SELECT Clasificacion 
							FROM AgrupadoresSpoolPND 
	...
	FETCH NEXT FROM ClasificacionJuntas INTO @sID
END
CLOSE ClasificacionJuntas
DEALLOCATE ClasificacionJuntas

DECLARE @TablaAvance TABLE(
	TipoId INT,
	SpoolID INT,
	Codigo CHAR(5),
...
DECLARE AvanceJuntas CURSOR FOR
	SELECT SpoolID FROM #TempSpool
OPEN AvanceJuntas
FETCH NEXT FROM AvanceJuntas INTO @sID
WHILE @@Fetch_Status = 0 BEGIN
	--SELECT @sID
	INSERT INTO @TablaAvance
		SELECT tj.TipoJuntaID,
				@sID,
				tj.Codigo,
				(
					SELECT COUNT(DISTINCT(jws.JuntaWorkstatusID)) FROM JuntaReportePnd jrp INNER JOIN ReportePnd rp on jrp.ReportePndID = rp.ReportePndID
					...
					WHERE js.SpoolID = @sID
					AND  (rp.TipoPruebaID = (CASE WHEN js.RequierePwht = 1 THEN 
													(SELECT TipoPruebaID FROM TipoPrueba WHERE TipoPruebaID = @TipoPruebaRTPOSTId)
												END
											)
						  OR rp.TipoPruebaID IN (SELECT TipoPruebaID FROM TipoPrueba WHERE Categoria = 'PND')
						)
					AND js.TipoJuntaID = tj.TipoJuntaID
					AND js.FabAreaID = @fabAreaId
					AND jrp.Aprobado = 1 
...
SELECT
	...
	(
		SELECT *
		FROM @Tabla Junta WHERE junta.SpoolID = sp.SpoolID
		FOR XML AUTO, ROOT('Juntas')
	),
	(
		SELECT *
		FROM @TablaClasificacion Junta WHERE Junta.SpoolID = sp.SpoolID
		FOR XML AUTO, ROOT('Juntas')
	),
	(
		SELECT *
		FROM @TablaAvance Junta WHERE Junta.SpoolID = sp.SpoolID
		FOR XML AUTO, ROOT('Juntas')
	),
	(
	...
	(
		...
		AND jws.JuntaWorkstatusID IN (SELECT JuntaWorkstatusID FROM JuntaSoldadura)
		AND js.FabAreaID = @fabAreaId
	),
	(
		SELECT COUNT(js.JuntaSpoolID) FROM JuntaSpool js
		WHERE js.SpoolID = sp.SpoolID
		AND (js.JuntaSpoolID IN (
				SELECT JuntaSeguimientoID1 FROM JuntaReportePnd
			)
			OR
			Js.JuntaSpoolID IN (
				SELECT JuntaSeguimientoID2 FROM JuntaReportePnd
			)
		)
	)
	FROM Spool sp
	...
	--DESPLEGAR TABLA
		...
		select g.GeneralSpoolID,
		   ...
		   id.InspeccionDimensionalInspector,
		   ...
		   tom.Despachador [GeneralDespachador],
			...
			tasp.AgrupadoresSpoolPNDJuntasSeguimiento,
