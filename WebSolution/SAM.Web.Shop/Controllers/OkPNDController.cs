...
{
    public class OkPNDController : AuthenticatedController
    {
        private static readonly ILog _logger = LogManager.GetLogger(typeof(OkPNDController));

        public OkPNDController(INavigationContext navContext) : base(navContext) { }
        ...
        [HttpGet]
        public ActionResult AddPND(SearchControlNumberModel model, WorkstatusModel shipment)
        {
        ...
        public ActionResult DeletePND(int id)
        {
            string spools = NavContext.GetCurrentNumbersControl();
        ...
        [HttpPost]
        public ActionResult SavePND(WorkstatusModel model)
        {
            ...
            if (ModelState.IsValid && ValidateModel(model))
            {
                ...
                foreach (NumeroControlBusqueda ncb in model.ControlNumberToProcess)
                {
                    ...
                    if (wkSpool != null)
                    {
                        if (wkSpool.FechaOkPnd.HasValue)
                        {
                ...
                try
                {
                    ...
                    {
                        OkPNDBO.Instance.GuardarOkPnds(Convert.ToDateTime(model.ProcessDate), lstSpoolsToProcess.ToArray(), SessionFacade.UserId);
                    }
        ...
        public ActionResult OverwritePND(WorkstatusModel model)
        {
            model.ControlNumberWhitProcess = new List<NumeroControlBusqueda>();
            model.ControlNumberToProcess = new List<NumeroControlBusqueda>();
            model.ControlNumberNotConditions = new List<NumeroControlBusqueda>();
        ...
        public ActionResult DeleteOkPNDResults(int id, WorkstatusModel model)
        {
            model.ControlNumberToProcess = new List<NumeroControlBusqueda>();
            model.ControlNumberWhitProcess = new List<NumeroControlBusqueda>();
            model.ControlNumberNotConditions = new List<NumeroControlBusqueda>();
