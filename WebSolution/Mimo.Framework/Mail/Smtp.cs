...
namespace Mimo.Framework.Mail
{
    public class Smtp
    {
        ...
        public void SendAttachFile(int smtpPort,
           string smtpHost,
                       string from,
                        string login,
                        string password,
                       ...
                       string fileName)
        {
            SendWithAttachFileName(smtpPort,
                smtpHost,
                from,
                 login,
                 password,
        ...
        /// <param name="fileName"></param>
        public void SendWithAttachFileName(int smtpPort,
            string smtpHost,
            string from,
                         string login,
                         string password,
                         ...
                         string fileName)
        {
            try
            {
                _logger.DebugFormat("Inicio SendWithAttachFileName");
                MailMessage mmMessage = new MailMessage();
                mmMessage.From = new MailAddress(from);
                ...
                string[] toMails = to.Split(new char[] { ';' });

                foreach (string email in toMails)
                {
                    mmMessage.To.Add(new MailAddress(email));
                }
                ...
                if (fileName != null)
                {
                    Attachment attachFileName = new Attachment(fileName);
                    mmMessage.Attachments.Add(attachFileName);
                }
                ...
                _logger.DebugFormat("fin SendWithAttachFileName");
            }
            catch (System.Exception ex)
            {
                _logger.Error("Error SendWithAttachFileName Function",
                              ex);
                throw new System.Exception("Smpt - Error SendWithAttachFileName Function",
                                    ex);
            }
        }
