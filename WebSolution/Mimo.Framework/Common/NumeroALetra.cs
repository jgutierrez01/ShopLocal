...
namespace Mimo.Framework.Common
{
    ...
    /// </summary>
    public sealed class NumeroALetra
    {
        ...
        private static string[,] _matrizIngles = new string[CENTENA + 1, 10]
                                                     {
                                                         {null," one", " two", " three", " four", " five", " six", " seven", " eigth", " nine"},
                                                         {" ten"," eleven"," twelve"," thirteen"," fourteen"," fifteen"," sixteen"," seventeen"," eighteen"," nineteen"},
                                                         {null,null," twenty"," thirty"," forty"," fifty"," sixty"," seventy"," eighty"," ninety"},
                                                         {null,null,null,null,null,null,null,null,null,null}
                                                     };

        public enum Moneda { Ninguno =0, Pesos=1, Dolares=2, Euros=3 };
        public enum Idioma { Ninguno=0, Espanol=1, Ingles=2 };
        private static string[] _firmaMoneda = { "MN", "USD", "EUR" };
        ...
        {
            return NumeroALetra.ToCardinal(Numero, CultureInfo.CurrentCulture);
        }
        ...
        /// <summary>
        /// Regresa el valor de la cantidad en Letra dependiendo del idioma
        /// y la moneda que se selecciona
        /// </summary>
        /// <param name="Numero">El número a convertir en letra</param>
        /// <param name="idioma">Idioma del texto</param>
        /// <param name="moneda">Tipo de moneda</param>
        /// <returns>Texto de la cantidad</returns>
        public static String MonedaALetra(Decimal Numero, Idioma idioma, Moneda moneda)
        {
            string tempMoneda = String.Empty;
            string separadorDecimal = String.Empty;
            string firmaMoneda = String.Empty;
            ...
            switch (moneda)
            {
                case Moneda.Pesos:
                    ...
                    break;
                case Moneda.Dolares:
                    tempMoneda = idioma == Idioma.Espanol ? " dólares" : " dollars";
                    firmaMoneda = " USD";
                    break;
                case Moneda.Euros:
                    tempMoneda = " euros";
            ...
            {
                separadorDecimal = tempMoneda;
                return Convertir(Numero, DecimalesDefault, separadorDecimal, MascaraSalidaDecimalDefault,
                                 true, LetraCapitalDefault, ConvertirDecimalesDefault, ApocoparUnoParteEnteraDefault,
                                 ApocoparUnoParteDecimalDefault, firmaMoneda);
            }
            else //idioma == Idioma.Ingles
            {
                separadorDecimal = tempMoneda;
                return ConvertirIngles(Numero, DecimalesDefault, separadorDecimal, MascaraSalidaDecimalDefault,
                                       true, LetraCapitalDefault, ConvertirDecimalesDefault, firmaMoneda);
            }
        }
        ...
        /// <param name="SeparadorDecimalSalida"></param>
        /// <param name="MascaraSalidaDecimal"></param>
        /// <param name="EsMascaraNumerica"></param>
        /// <param name="LetraCapital"></param>
        /// <param name="ConvertirDecimales"></param>
        /// <param name="ApocoparUnoParteEntera"></param>
        /// <param name="ApocoparUnoParteDecimal"></param>
        /// <param name="firmaMoneda"></param>
        /// <returns></returns>
        private static String Convertir(Decimal Numero, Int32 Decimales, String SeparadorDecimalSalida, 
                                        ...
                                        Boolean ApocoparUnoParteDecimal, String firmaMoneda)
        {
            Int64 Num;
            ...
            if (Num >= 1000000000000 || Num < 0) throw new ArgumentException("El número '" + 
                                                                             Numero.ToString() + "' excedió los límites del conversor: [0;1.000.000.000.000)");
            if (Num == 0)
            ...
            if (LetraCapital)
                return Resultado[1].ToString().ToUpper() + Resultado.ToString(2, Resultado.Length - 2) + firmaMoneda;
            else
                return Resultado.ToString().Substring(1) + firmaMoneda;
        }
        ...
        /// <param name="firmaMoneda"></param>
        /// <returns></returns>
        private static String ConvertirIngles(Decimal Numero, Int32 Decimales, String SeparadorDecimalSalida,
                                              String MascaraSalidaDecimal, Boolean EsMascaraNumerica, Boolean LetraCapital,
                                              Boolean ConvertirDecimales, String firmaMoneda)
        {
            Int64 Num;
            Int32 terna, centenaTerna, decenaTerna, unidadTerna, iTerna;
            String cadTerna;
            ...
            else
            {
                ...
                while (Num > 0)
                {
                    ...
                    if ((decenaTerna > 0) && (decenaTerna < 10))
                        cadTerna = _matrizIngles[UNI, unidadTerna] + cadTerna;
                    else if ((decenaTerna >= 10) && (decenaTerna < 20))
                        cadTerna = cadTerna + _matrizIngles[DIECI, unidadTerna];
                    else if (decenaTerna == 20)
                        cadTerna = cadTerna + " twenty";
                    else if ((decenaTerna > 20) && (decenaTerna < 30))
                        cadTerna = " twenty-" + _matrizIngles[UNI, unidadTerna].Trim();
                    else if ((decenaTerna >= 30) && (decenaTerna < 100))
                        if (unidadTerna != 0)
                            cadTerna = _matrizIngles[DECENA, (Int32)(decenaTerna / 10)] + "-" + _matrizIngles[UNI, unidadTerna].Trim() + cadTerna;
                        else
                            cadTerna += _matrizIngles[DECENA, (Int32)(decenaTerna / 10)];                

                    if ((Int32)(terna / 100) > 1) 
                        cadTerna = _matrizIngles[UNI, (Int32)(terna / 100)] + " hundred " + cadTerna;
            ...
            if (Decimales > 0)
            {
                ...
                {
                    Boolean esMascaraDecimalDefault = MascaraSalidaDecimal == MascaraSalidaDecimalDefault;
                    Resultado.Append(Convertir((Decimal)EnteroDecimal, 0, null, null, EsMascaraNumerica, false, false, !EsMascaraNumerica, false, "") + " "
                                     + (EsMascaraNumerica ? "" : MascaraSalidaDecimal));
                }
                else
                    if (EsMascaraNumerica) Resultado.Append(EnteroDecimal.ToString(MascaraSalidaDecimal));
