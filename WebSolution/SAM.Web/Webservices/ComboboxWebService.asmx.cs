...
namespace SAM.Web.Webservices
{
    ...
    public class ComboboxWebService : WebService
    {
        ...
        /// <summary>
        /// Obtiene el listado de cortadores por proyecto.
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod]
        public IEnumerable<RadCortador> ListaCortadoresPorProyecto(RadComboBoxContext context)
        {
            int proyectoId = 0;
            if (context.Keys.Contains("ProyectoID"))
            {
            ...
            return CortadorBO.Instance.ObtenerPorProyectoParaCombo(proyectoId, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST);
        }
        ...
        [WebMethod]
        public IEnumerable<RadInspector> ListaInspectoresPorTaller(RadComboBoxContext context)
        {
            int tallerID = -1;

            if (context.Keys.Contains("TallerID"))
            {
                tallerID = context["TallerID"].ToString().SafeIntParse();
            }

            return InspectorBO.Instance.ObtenerPorTallerParaCombo(tallerID, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST);
        }


        /// <summary>
        /// Obtiene los Soldadores por proyecto en base a lo que el usuario va escribiendo en el combo.
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod]
        ...
        /// <summary>
        /// Obtiene los Despachadores por proyecto en base a lo que el usuario va escribiendo en el combo.
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod]
        public IEnumerable<RadDespachador> ListaDespachadoresPorProyecto(RadComboBoxContext context)
        {
            int proyectoID = -1;
            if (context.Keys.Contains("ProyectoID"))
            {
            ...
            return DespachadorBO.Instance.ObtenerPorProyectoParaCombo(proyectoID, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST);
        }
        ...
        [WebMethod]
        public IEnumerable<MyItemData> ListaNumerosControlYJunta(RadComboBoxContext context)
        {
            int ProyectoId = context["ProyectoID"].SafeIntParse();
            string jwsIDs = context["JuntaWorkstatusIDs"].SafeStringParse();

            List<Simple> lista = JuntaSpoolBO.Instance.ObtenerJuntasPorProyectoYNumeroJunta(ProyectoId, context.Text , context.NumberOfItems, ELEMENTOS_POR_REQUEST, jwsIDs);
            
            return lista.ConvertAll<MyItemData>(
               new Converter<Simple, MyItemData>(
                   delegate(Simple simple)
                   {
                       return new MyItemData(simple.Valor, simple.ID.ToString());
        ...
        /// <summary>
        /// Lista solo los numeros de control que tienen registro en WorkstatusSpool
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod(EnableSession = true)]
        public IEnumerable<MyItemData> ListaNumerosControlLiberacionCalidad(RadComboBoxContext context)
        {
            ...
            List<Simple> lst =
                OrdenTrabajoSpoolBO.Instance
                                   .ObtenerNumerosDeControlLiberacionCalidad(proyectoID,
        ...
        /// <summary>
        /// OBTIENE LA LISTA DE CUADRANTES DISPONIBLES ASIGNADOS AL PATIO DEL PROYECTO
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod(EnableSession = true)]
        ...
        {
            int? proyectoID = UtileriasRadCombo.ObtenId(context, "ProyectoID");
            List<Simple> lst = CuadranteBO.Instance.ObtenerCuadrantesProyecto(proyectoID, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST).ToList();
        ...
        //[WebMethod(EnableSession = true)]
        //public IEnumerable<MyItemData> ListadoDeTalleresPorProyecto(RadComboBoxContext context)
        //{
        //    //int? proyectoID = UtileriasRadCombo.ObtenId(context, "ProyectoID)");
        //    int? proyectoID = (int)Session["TempProyectoID"];
        //    List<Simple> lst = TallerBO.Instance.ObtenerTalleresPorProyecto(proyectoID, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST).ToList();
        //    return lst.ConvertAll<MyItemData>(
        //        new Converter<Simple, MyItemData>(
        //            delegate(Simple simple)
        //            {
        ...
        /// <summary>
        /// Obtiene los numeros de embarque de un proyecto
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        [WebMethod(EnableSession = true)]
        public IEnumerable<MyItemData> ListaNumerosEmbarquePorProyecto(RadComboBoxContext context)
        {
            int? proyectoID = UtileriasRadCombo.ObtenId(context, "ProyectoID");
            List<Simple> lst = EmbarqueBO.Instance.ObtenerNumerosDeEmbarquePorProyecto(proyectoID, context.Text, context.NumberOfItems, ELEMENTOS_POR_REQUEST).ToList();
        ...
        [WebMethod]
        public IEnumerable<RadNumeroUnicoParaDespacho> NumerosUnicosParaDespachoDesdeArmado(RadComboBoxContext context)
        {
            int juntaSpoolId = context["juntaSpoolID"].SafeIntParse();
            int skip = context.NumberOfItems;
            int take = ELEMENTOS_POR_REQUEST;
            IEnumerable<RadNumeroUnicoParaDespacho> numerosUnicos =
                NumeroUnicoBO.Instance.AccesoriosAfinesParaDespachoDesdeArmado(juntaSpoolId, context.Text, skip, take, false, 1);
            return numerosUnicos;
        }

        [WebMethod]
        public IEnumerable<RadNumeroUnicoParaDespacho> NumerosUnicosParaDespachoDesdeArmadoNumUnico2(RadComboBoxContext context)
        {
            int juntaSpoolId = context["juntaSpoolID"].SafeIntParse();
            int skip = context.NumberOfItems;
            int take = ELEMENTOS_POR_REQUEST;
            IEnumerable<RadNumeroUnicoParaDespacho> numerosUnicos =
                NumeroUnicoBO.Instance.AccesoriosAfinesParaDespachoDesdeArmado(juntaSpoolId, context.Text, skip, take, false, 2);
            return numerosUnicos;
        }
        ...
        public IEnumerable<RadMaterialParaCorte> ListaMaterialesPorNumeroControl(RadComboBoxContext context)
        {
            ...
            if (context.ContainsKey("Etiqueta"))
            {
                etiqueta = context["Etiqueta"].ToString();
            }

            string[] split = etiqueta.Split(new Char[] { ';' });
            List<int> lstEtiquetas = new List<int>();

             foreach (var a in split) {
                 if (!string.IsNullOrEmpty(a)) {
                     lstEtiquetas.Add(a.SafeIntParse());
                 }                 
             }

            IQueryable<int> iqEtiquetas = lstEtiquetas.AsQueryable();
            IQueryable<RadMaterialParaCorte> result = MaterialSpoolBO.Instance.ListaMaterialesPorNumeroControl(ordenTrabajoSpoolID, numeroUnicoID, segmento, context.Text, skip, take).AsQueryable();
           
            IQueryable<RadMaterialParaCorte>resultfinal= from a in result
                                                         where !iqEtiquetas.Contains(a.MaterialSpoolID)
                                                         select a;
            return resultfinal;
        }
