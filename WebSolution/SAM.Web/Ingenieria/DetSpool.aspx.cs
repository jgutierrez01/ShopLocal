    ...
    public partial class DetSpool : SamPaginaPrincipal
    {
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {             
                ...
                {
                    //es un spool sin ODT
                    //Si se eliminaron Juntas
                    if (ctrlJunta.lstJuntasEliminadasIds.Count > 0)
                    {
                        //Eliminar Agrupadores de Juntas
                        SpoolBO.Instance.EliminarAgrupadoresPorJunta(ctrlJunta.lstJuntasEliminadasIds);
                    }
                    SpoolBO.Instance.Guarda(spool);
                    string url = String.Format(WebConstants.IngenieriaUrl.REDIRECT_NOMBRADO_SPOOL, spool.ProyectoID);
                    Response.Redirect(url);
