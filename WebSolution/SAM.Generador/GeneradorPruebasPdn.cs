...
namespace SAM.Generador
{
    public class GeneradorPruebasPdn
    {
        ...
        public void Inicia()
        {
            ...
            List<GrdRequisiciones> lstJuntas = ObtenerJuntasParaPnd(proyecto.ProyectoID, tipoPrueba).ToList();
            ...
            while(true)
            {
                ...
                if (lstJwIdParaRechazo.Count > 0)
                {
                    ...
                    {
                        TipoRechazoEnum tipoRechazo = (TipoRechazoEnum)(rnd.Next(1, 4) == 1 ? 1 : 2);
                        List<JuntaReportePndSector> sectores = new List<JuntaReportePndSector>();
                        List<JuntaReportePndCuadrante> cuadrantes = new List<JuntaReportePndCuadrante>();
                        int defectoID = -1;
                        #region Tipo Rechazo

                        switch (tipoRechazo)
                        {
                            case TipoRechazoEnum.Sector:

                                int cntSectoresDefectuosos = rnd.Next(1, 5);

                                for (int i = 0; i < cntSectoresDefectuosos; i++)
                                {
                                    defectoID = defectos.Where(x => x.TipoPruebaID == (int)tipoPrueba)
                                                        .Select(x => x.DefectoID)
                                                        .OrderBy(y => rnd.NextDouble())
                                                        .First();

                                    sectores.Add(new JuntaReportePndSector
                                    {
                                        DefectoID = defectoID,
                                        FechaModificacion = DateTime.Now,
                                        UsuarioModifica = _userID,
                                        Sector = (i + 1).ToString(),
                                        SectorInicio = ((char)(baseAscii + i)).ToString(),
                                        SectorFin = ((char)(baseAscii + i + 1)).ToString()
                                    });
                                }

                                break;
                            case TipoRechazoEnum.Cuadrante:

                                int cntCuadrantesDefectuosos = rnd.Next(1, 5);

                                for (int i = 0; i < cntCuadrantesDefectuosos; i++)
                                {
                                    defectoID = defectos.Where(x => x.TipoPruebaID == (int)tipoPrueba)
                                                        .Select(x => x.DefectoID)
                                                        .OrderBy(y => rnd.NextDouble())
                                    ...
                                    cuadrantes.Add(new JuntaReportePndCuadrante
                                    {
                                        DefectoID = defectoID,
                                        FechaModificacion = DateTime.Now,
                                        UsuarioModifica = _userID,
                                        Cuadrante = (i + 1).ToString(),
                                        Placa = ((char)(baseAscii + i)).ToString()
                                    });
                                }
        ...
        {
            //Primero generar las aprobaciones
            ...
            try
            {
                ...
                ReportesBL.Instance.GuardaReportePND(reporte,
                                                        juntaReporte,
                                                        new List<JuntaReportePndSector>(),
                                                        new List<JuntaReportePndCuadrante>(),
                                                        jwIdsParaAprobacion,
                                                        rIdsParaAprobacion,
                                                        _userID);
        ...
        private void GenerarRechazo(int tipoPrueba, Proyecto proyecto, Stopwatch sw, DateTime hoy, List<JuntaReportePndSector> sectores, List<JuntaReportePndCuadrante> cuadrantes, TipoRechazoEnum tipoRechazo, string jwIdsParaRechazo, string rIdsParaRechazo, string numReporte)
        {
            //Primero generar las aprobaciones
            ReportePnd reporte = new ReportePnd
            {
        ...
        private static TipoPruebaEnum SolicitaTipoPrueba()
        {
            while (true)
            {
                KeyboardUtils.ImprimeInline("Tipo de prueba? (PT-Post-TT, PT, RT, UT, RT-Post-TT): ");
        ...
        private List<GrdRequisiciones> ObtenerJuntasParaPnd(int proyectoID, int tipoPruebaID)
        {
